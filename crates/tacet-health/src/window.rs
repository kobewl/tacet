...
impl WindowPolicy {
    ...
    /// 判断当前是否适合打扰。
    ///
    /// 判断顺序（**顺序本身就是规则**，改动前请读完每一条的理由）：
    ///
    /// 1. **人不在 → 关闭**。排在最前面，优先于紧急豁免。
    /// 2. **紧急豁免**：需求拉满时，剩下的顾虑（深夜、安静期）都让路。
    /// 3. 刚打扰过 → 关闭
    /// 4. 深夜 → 关闭
    /// 5. 其余 → 打开
    ///
    /// ## 为什么「人不在」必须排在紧急豁免之前
    ///
    /// 早期版本的顺序是反的：紧急豁免在最前面直接 `return open`，
    /// 于是「人不在」这个判断**永远不会被执行到**。当时的想法是
    /// 「一个『你连续工作 4 小时了』不能等」——但漏了一种真实情况：
    ///
    /// > 需求分数是**按墙上时钟**算的。用户晚上 22 点离开、屏幕熄灭，
    /// > 应用仍在运行（屏幕休眠 ≠ 系统休眠），于是「距上次休息」
    /// > 每分钟都在增长。到凌晨 4 点这个数已经有 10 小时，
    /// > 分数远超紧急阈值 —— 然后**对着熄屏的电脑弹了全屏提醒**。
    ///
    /// 真实数据里，用户睡着的这一夜被弹了 10 次全屏提醒
    /// （04:17 / 04:47 / 05:33 …… 09:04，每半小时一次，
    /// 间隔正是休息类提醒的冷却时长）。
    ///
    /// 这条规则背后是一个比「紧急」更基本的常识：
    /// **提醒是给在场的人看的**。人不在，提醒就没有接收者 ——
    /// 无论需求多紧急，都不该对着空椅子说话。
    ///
    /// 紧急豁免要豁免的是「深夜」和「安静期」这类**时机上的顾虑**
    /// （它们的前提是「人在，只是时机不合适」），而不是「人在不在」
    /// 这个**前提本身**。
    pub fn evaluate(
        &self,
        now: Timestamp,
...
mod tests {
    ...
    fn 紧急需求可以突破深夜与安静期() {
        ...
        // 前提是**人在** —— 深夜还在用电脑的人，恰恰最需要被劝去睡觉。
        let policy = WindowPolicy::new();

        let window = policy.evaluate(
            at_hour(3),
            &active_context(),
            Some(at_hour(3).saturating_sub_millis(30_000)), // 30 秒前刚提醒过
            300,
            true,
        );
        ...
        assert!(
            window.is_open,
            "人在场时，紧急需求应当豁免深夜与安静期这两项时机顾虑"
        );
    }

    /// 回归测试：紧急需求**不能**突破「人不在」这一条。
    ///
    /// ## 这个 bug 是怎么被发现的
    ///
    /// 用户报「我一直在息屏，还提醒休息」。查数据发现他睡着的这一夜
    /// 被弹了 10 次全屏提醒（04:17 到 09:04，每半小时一次）。
    ///
    /// 根因就是这条测试守的那个判断顺序：早期版本把「紧急豁免」放在
    /// 最前面，直接 `return open`，于是「人不在」那段代码永远执行不到。
    /// 而需求分数是按墙上时钟算的 —— 用户 22 点离开、屏幕熄灭，
    /// 到凌晨 4 点「距上次休息」已经累积了 10 小时，分数远超紧急阈值。
    ///
    /// 「提醒是给在场的人看的」比「需求有多紧急」更基本：
    /// 人不在，提醒就没有接收者。
    #[test]
    fn 紧急需求不能突破人不在() {
        let policy = WindowPolicy::new();
        let mut ctx = active_context();
        ctx.idle_seconds = 3600; // 人走了一小时（屏幕熄灭、去睡觉）

        let window = policy.evaluate(
            at_hour(4),
            &ctx,
            Some(at_hour(4).saturating_sub_millis(30 * MINUTE)),
            300,
            true, // 需求已经拉满（离开期间「距上次休息」一直在涨）
        );

        assert!(
            !window.is_open,
            "人都走了一小时了，再紧急也不该对着空椅子弹全屏提醒"
        );
        assert_eq!(
            window.closed_reason,
            Some(WindowClosedReason::Away),
            "关闭原因要写清楚是「人不在」，而不是让人以为是深夜或限流"
        );
    }

    /// 回归测试：人刚回来（空闲还没超过阈值）时，紧急提醒仍然发得出去。
    ///
    /// 「人不在」的判据是**空闲超过阈值**，不是「曾经离开过」。
    /// 用户刚坐下、动了鼠标，空闲秒数归零 —— 此刻他在场，
    /// 一条紧急提醒应该被允许。
    #[test]
    fn 人刚回来时紧急提醒仍然可以发出() {
        let policy = WindowPolicy::new();
        let mut ctx = active_context();
        ctx.idle_seconds = 2; // 刚动过鼠标

        let window = policy.evaluate(at_hour(4), &ctx, None, 300, true);

        assert!(window.is_open, "已经在电脑前了，深夜也该把紧急提醒发出去");
    }
